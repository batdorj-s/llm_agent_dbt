    ...
    <div className="absolute right-0 top-0 bottom-0 w-[480px] border-l border-border bg-sidebar z-50 flex flex-col shadow-xl animate-slide-in-right">
      <div className="flex items-center justify-between px-4 py-3 border-b border-border shrink-0">
        <div className="flex items-center gap-2 min-w-0">
          <span className="text-[10px] font-bold text-foreground/50 uppercase tracking-wider shrink-0">Урьдчилсан харагдац</span>
        ...
        <div className="flex-1 overflow-auto p-4 space-y-3 animate-fade-in-up">
          <div className="flex items-center justify-between">
            <div className="flex gap-2 items-start">
              ...
              <div>
                <p className="text-[10px] font-bold text-foreground/50 uppercase tracking-wider mb-1">Баримт</p>
            ...
            {previewHasDownload && previewFileId && (
              <a href={`/api/admin/files/${previewFileId}/download`}
                ...
                <FileText className="w-3 h-3" /> Татах
              </a>
            )}
          </div>
          {previewDescription && (
            <div className="border-t border-border pt-2">
              <p className="text-[10px] font-bold text-foreground/50 uppercase tracking-wider mb-1">Тайлбар</p>
              <p className="text-xs text-foreground/70 whitespace-pre-wrap">{previewDescription}</p>
            </div>
          )}
          <div className="border-t border-border pt-2">
            <p className="text-[10px] font-bold text-foreground/50 uppercase tracking-wider mb-1">Агуулга</p>
      ...
      {previewData.length > 0 && (
        <div className="px-4 py-2 border-t border-border text-[9px] text-foreground/40 shrink-0 flex items-center justify-between">
          <span>{previewData.length} мөр харуулав (эхний 20)</span>
        </div>
      )}
    </div>
  );
