...
describe("RateLimiter", () => {
    ...
    it("allows requests up to the limit", () => {
        ...
        expect(limiter.check("k1")).toEqual({ allowed: true, remaining: 2, resetInMs: 60_000 });
        expect(limiter.check("k1")).toEqual({ allowed: true, remaining: 1, resetInMs: 60_000 });
        expect(limiter.check("k1")).toEqual({ allowed: true, remaining: 0, resetInMs: 60_000 });
    });
    ...
    it("stats returns correct count and remaining", () => {
        const limiter = new RateLimiter({ maxRequests: 5, windowMs: 60_000 });
        expect(limiter.stats("unknown")).toEqual({ requests: 0, remaining: 5 });
        limiter.check("k1");
        limiter.check("k1");
        expect(limiter.stats("k1")).toEqual({ requests: 2, remaining: 3 });
    });

    it("sliding window evicts old entries", () => {
        const limiter = new RateLimiter({ maxRequests: 3, windowMs: 10_000 });

        limiter.check("k1"); // t=0
        vi.advanceTimersByTime(5_000);
        limiter.check("k1"); // t=5s
        limiter.check("k1"); // t=5s
        expect(limiter.check("k1").allowed).toBe(false); // 3 in window

        vi.advanceTimersByTime(5_001); // t=10_001ms — first entry (t=0) is evicted
        expect(limiter.check("k1").allowed).toBe(true);
    });

    it("startCleanup removes stale keys", () => {
        ...
        expect(limiter.stats("stale")).toEqual({ requests: 0, remaining: 2 });
        clearInterval(interval);
    });

    it("startCleanup keeps active entries but prunes expired timestamps", () => {
        const limiter = new RateLimiter({ maxRequests: 5, windowMs: 10_000 });
        const interval = limiter.startCleanup(1_000);

        limiter.check("active"); // t=0
        vi.advanceTimersByTime(9_000);
        limiter.check("active"); // t=9s — still in window
        vi.advanceTimersByTime(2_000); // t=11s — first entry expired
        vi.advanceTimersByTime(1_000); // cleanup fires

        const s = limiter.stats("active");
        expect(s.requests).toBe(1); // only the t=9s entry remains
        expect(s.remaining).toBe(4);
        clearInterval(interval);
    });

    it("handles maxRequests = 0 (block all)", () => {
    ...
    it("handles large burst of requests", () => {
        ...
        expect(limiter.stats("burst")).toEqual({ requests: 1000, remaining: 0 });
    });

    it("send correct resetInMs for blocked request", () => {
        const limiter = new RateLimiter({ maxRequests: 2, windowMs: 10_000 });
        limiter.check("k1"); // t=0
        limiter.check("k1"); // t=0
